...
export function ForSalonsWelcome({ salonId }: { salonId?: string }) {
  ...
  return (
    <div className="mx-auto w-full max-w-xl">
      <div className="rounded-xl border border-border/70 bg-card p-5 shadow-sm shadow-black/[0.03] sm:p-6">
        <div className="flex size-12 items-center justify-center rounded-full bg-primary/10 text-primary">
          <CheckCircle2Icon className="size-6" aria-hidden />
        </div>

        {localDev ? (
          <div className="mt-5 rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-foreground/75">
            <p className="font-medium text-foreground">Start Glammzo CRM first</p>
            <p className="mt-1.5 leading-relaxed">
              Run <code className="rounded bg-background/80 px-1.5 py-0.5 text-xs">npm run dev</code>{" "}
              in <code className="rounded bg-background/80 px-1.5 py-0.5 text-xs">glamzzo-crm</code> (
        ...
        {salonId ? (
          <p className="mt-5 text-xs text-foreground/40">Workspace ID · {salonId}</p>
        ) : null}
      </div>
